<template>
  <div
    ...
    <main class="mt-24 max-w-container-max mx-auto px-gutter md:px-0 pb-stack-xl">
      <!-- Hero Section -->
      ...
      <section class="mb-stack-lg">
        <div class="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/20 p-6 md:p-8">
          <h2 class="font-headline-md text-headline-md text-on-surface mb-stack-lg flex items-center gap-2">
            核心技术栈 <span class="w-12 h-[1px] bg-outline-variant"></span>
          </h2>
          <div v-if="skillsLoading" class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div v-for="i in 4" :key="i" class="bg-surface-container rounded-xl p-4 animate-pulse">
              <div class="h-6 bg-surface-container-high rounded w-1/2 mb-2"></div>
              <div class="h-4 bg-surface-container-high rounded w-3/4"></div>
            </div>
          </div>
          <div v-else-if="skillsError" class="text-error text-sm py-8 text-center">
            <p>{{ skillsError }}</p>
          ...
          <div v-else class="space-y-6">
            ...
            <div v-if="frontendSkills.length > 0">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div v-for="skill in frontendSkills" :key="skill.id" @click="showSkillDetail(skill)"
            ...
            <div v-if="backendSkills.length > 0">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div v-for="skill in backendSkills" :key="skill.id" @click="showSkillDetail(skill)"
                  class="cursor-pointer">
                  <SkillCard :skill="skill" />
                </div>
              </div>
            ...
            <div v-if="devopsSkills.length > 0">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div v-for="skill in devopsSkills" :key="skill.id" @click="showSkillDetail(skill)"
                  class="cursor-pointer">
                  <SkillCard :skill="skill" />
                </div>
              </div>
            ...
            <div v-if="languageSkills.length > 0">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div v-for="skill in languageSkills" :key="skill.id" @click="showSkillDetail(skill)"
                  class="cursor-pointer">
                  <SkillCard :skill="skill" />
                </div>
              </div>
      ...
      <section>
        <div class="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/20 p-6 md:p-8">
          ...
          <div v-if="postsLoading" class="space-y-stack-lg">
            <div v-for="i in 3" :key="i" class="bg-surface-container rounded-xl p-6 animate-pulse">
              <div class="h-6 bg-surface-container-high rounded w-2/3 mb-3"></div>
              <div class="h-4 bg-surface-container-high rounded w-full mb-2"></div>
              <div class="h-4 bg-surface-container-high rounded w-4/5"></div>
            </div>
          </div>
          <div v-else-if="postsError" class="text-error text-sm py-8 text-center">
            <p>{{ postsError }}</p>
            <button @click="fetchSkills"
              class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors">
              重试
          ...
          <div v-else-if="recentPosts.length === 0" class="text-on-surface-variant text-center py-8">
            暂无文章
          </div>
          <div v-else class="space-y-stack-lg">
            <BlogPostCard v-for="post in recentPosts" :key="post.id" :post="post" />
          </div>
        </div>
      </section>
    </main>
    ...
    <Teleport to="body">
      <transition name="dialog">
        <div v-if="showSkillDialog" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4"
          @click.self="closeSkillDialog">
          <div class="bg-surface-container-lowest rounded-2xl p-6"
            :class="currentWidth === 'mobile' ? 'w-[80%]' : 'w-full max-w-[600px]'">
            <div class="flex items-center justify-between mb-6">
              <h2 class="font-headline-md text-headline-md text-on-surface">
                {{ selectedSkill?.name }}
              </h2>
              <button @click="closeSkillDialog"
            ...
            <div v-if="selectedSkill" class="space-y-4">
              <div class="flex items-center gap-3 mb-4">
                <span class="px-3 py-1 bg-surface-container-high text-on-surface-variant text-sm rounded-full">
              ...
              <div class="mb-6">
                <div class="h-2 bg-surface-container-high rounded-full overflow-hidden">
                  <div class="h-full bg-primary rounded-full transition-all"
                    :style="{ width: `${selectedSkill.level * 10}%` }"></div>
                </div>
              </div>

              <!-- Description -->
              <div v-if="selectedSkill.description" class="text-on-surface-variant leading-loose whitespace-pre-line">
...
const languageSkills = computed(() => skills.value.filter(s => s.category === 'Language'));
