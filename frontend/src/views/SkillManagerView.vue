  ...
  <div class="min-h-screen bg-gradient-to-b from-surface-container-low to-surface font-body-md text-body-md">
    ...
    <main class="max-w-4xl mx-auto px-gutter py-stack-lg mt-stack-lg">
      ...
      <div class="grid grid-cols-1 gap-4 mt-stack-lg" :class="{ 'md:grid-cols-2': skills.length > 0 }">
        ...
        <!-- Skill Cards -->
        <div v-if="skills.length > 0" v-for="skill in skills" :key="skill.id"
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <h3 class="font-headline-md text-headline-md text-on-surface">{{ skill.name }}</h3>
              <span
          ...
          <!-- Description Preview -->
          <div v-if="skill.description" class="mt-3 text-sm text-on-surface-variant line-clamp-2">
            {{ skill.description }}
          </div>
        </div>

        <div v-else-if="skillsError"
              ...
              <form @submit.prevent="saveSkill" class="space-y-4">
                ...
                <div>
                  ...
                  <div class="flex flex-wrap gap-3 mb-2">
                    ...
                    <button type="button" @click="formData.icon = ''" :class="[
                    ...
                    <button v-for="svgIcon in svgIcons" :key="svgIcon.name" type="button"
                      @click="formData.icon = svgIcon.file" :class="[
                        'w-12 h-12 rounded-lg border-2 flex items-center justify-center transition-all p-2',
                        formData.icon === svgIcon.file
                          ? 'border-primary bg-primary/10'
                          : 'border-outline-variant hover:border-primary/50'
                      ]" :title="svgIcon.name">
                      <img :src="getIconUrl(svgIcon.file)" :alt="svgIcon.name" class="w-full h-full object-contain" />
                    </button>
                  </div>
                  <p class="text-xs text-on-surface-variant">点击选择图标，或留空使用默认图标</p>
                </div>
      ...
      <!-- 删除确认对话框 -->
      <DeleteConfirm v-model:visible="showDeleteConfirm" title="确认删除" message="确定要删除这个技能吗？" sub-message="此操作无法撤销，请谨慎操作。"
...
const saveSkill = async () => {
  ...
  try {
    const url = editingSkill.value ? `/api/skills/${editingSkill.value.id}` : '/api/skills';
